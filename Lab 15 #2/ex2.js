...
app.post("/check_login", function (request, response) {
    ...
    if (typeof userdata[login_username] != 'undefined') {
        ...
        response.cookie('username', login_username).send(`${login_username} is logged in with data ${JSON.stringify(quantity_str)} on ${theDate}`);
        return;
    } else {
    ...
    if (typeof request.cookies.username != `undefined`) {
        response.send(`Welcome back ${request.cookies.username}!` + `<br>` + `You last logged in on ${request.session.last_login_time}` );
    }
});
